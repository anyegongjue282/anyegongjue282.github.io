...
函数体里面用到了函数体外部的两个变量$closure, $accessories（注意这两个变量并非是闭包的
...
###第二次迭代
constructClosure的入参，$closure = $closure1（第一次迭代的结果），$accessories = "拉链"。
又形成了一个新的闭包，这个闭包被赋值给了变量$takeOffClothes。这个闭包执行的动作包括：
解开 拉链（继承自外部变量$accessories的值是拉链），脱掉衣服，再调用上一次迭代形成的闭包
$closure1。可以看到，这次形成的闭包其行为又有了不同。

我们可以这么来理解这两次迭代。一开始，我们需要处理的场景很简单，只穿了内衣，只需要脱掉
...
形成一个更大的闭包。这对应了上面的第二次迭代。

![闭包的迭代][closure_iteration]

可以看到，一个比较复杂的处理过程，我们通过闭包的迭代进行了抽象和简化，而闭包的两个特点
可以支持我们进行这样的抽象和简化。最终形成的闭包$takeOffClothes就好像是从里到外穿上3件
不同的衣服，描述好了我们需要解决的场景。

    $takeOffClothes = array_reduce($accessories, constructClosure,  $takeOffUnderWear);

上面的语句，我们只是得到了一个闭包，相当于得到了一个解决方案，但是我们只是得到了方案，
还没有解决问题。再想想闭包的本质是什么？对，匿名函数，函数就需要调用执行。最后我们需
要调用一下最终形成的闭包$takeOffClothes，这样就大功告成了。

运行结果如下。

![运行结果][example1_result]

通过这个例子，我们再来重新认识一下闭包这个词。闭包是匿名函数，包的是什么呢？包的是函数体
...
就好像是函数的内部代码一样，外部是看不见的。这么看来，叫“闭包”还是比较贴切的。

##例子：把大象放进冰箱

现在再回过头来看一下开头提到的把大象放进冰箱的问题。把大象放进冰箱的3个步骤看起来没有太多
的相似之处，也可以用闭包来解决吗？答案是可以。
